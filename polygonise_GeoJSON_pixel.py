        ...
        for individual_raster_file in raster_files:
            ...
            try: # Inner try-except for individual file processing
                with rasterio.open(individual_raster_file) as src:
                    ...
                    # Define a mask where True means the pixel is valid (not nodata)
                    valid_data_mask = (image != nodata_fill_value)

                    # Extract shapes (polygons) from the original image, using the valid_data_mask
                    # The 'v' in (s,v) will be the actual pixel value.
                    results = [
                        {'properties': {'raster_val': v}, 'geometry': s}
                        for i, (s, v) in enumerate(
                            shapes(image, mask=valid_data_mask, transform=transform)
                        )
                    ]

                    if not results:
                        logging.info(f"No valid data polygons found in {base_name} (excluding nodata value: {nodata_fill_value}) to polygonize.")
                        continue # Skip to the next file

                    geometries = [shape(result['geometry']) for result in results]
                    pixel_values = [result['properties']['raster_val'] for result in results]
                    gdf_polygons = gpd.GeoDataFrame({'value': pixel_values, 'geometry': geometries}, crs=crs)
